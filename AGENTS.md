...
- **Tests**: `bun test` corre los unitarios de WhatsApp-Bot (hoy: `endpointSchema.test.ts` — validación/repregunta/descripciones — y `answerParser.service.test.ts` — atajo determinista). `tsconfig.json` excluye `*.test.ts` para que `tsc` no necesite `bun:test`
